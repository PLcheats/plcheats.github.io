...
const ipgeolocation = 'https://api.ipgeolocation.io/ipgeo?apiKey=85c0c9dcaef44dea96ecdf7acad8186e';
...
$(document).ready(() => {
    ...
    app.titleChanger(['Fuck', 'Easy', 'AntiCheat', 'FuckEAC', 'FuckEasyAntiCheat', '#LunarPL', 'PLcheats', 'Song name: KUSO GVKI - Cinnamon (feat. Apfel) (splash! Mag Premiere)']);
...
$.getJSON(ipgeolocation, (data) => {
    writeLine(['Authenticating...', "Granting access to <span style='font-size: 14px; color: #06d;'>[unknown]</span>..."], 30, () => {
        ...
        writeLine([`Access granted! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Welcome back, <i style='color: #0f0'>${ip}</i>! By the way, nice to see someone from ${country} here!`], 30, 500, () => {
            ...
            writeLine([`<i style='color: #F62459'>PLcheats $$$</i>`], 120, 500, () => {
                timeouts.push(
                    setTimeout(() => {
                        if (app.skippedIntro) return;
